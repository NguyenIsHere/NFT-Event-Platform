...
if (SERVICE_TYPE !== 'ticket') {
  console.error(
    `FATAL ERROR: SERVICE_TYPE in .env for ${SERVICE_NAME}. Expected 'ticket', got '${SERVICE_TYPE}'`
  )
  process.exit(1)
}
if (!MONGO_URI) {
...
console.log(`${SERVICE_NAME}: Loading proto from ${TICKET_PROTO_PATH}`)
const mainPackageDefinition = protoLoader.loadSync(TICKET_PROTO_PATH, {
  keepCase: true,
  longs: String,
  enums: String,
  defaults: true,
    ...
    (err, boundPort) => {
      ...
      consul.agent.service
        .register({
          ...
          port: parseInt(PORT),
          tags: ['grpc', 'nodejs', SERVICE_NAME, 'ticket'],
          check: check
        })
        .then(() => {
          console.log(
