...
console.log(`TicketService: Loading user proto from ${USER_PROTO_PATH}`)
...
console.log(`TicketService: User client connecting to ${USER_SERVICE_ADDRESS}`)
...
const testConnection = () => {
  console.log(
    `TicketService: Testing connection to User service at ${USER_SERVICE_ADDRESS}`
  )
  ...
  userServiceClient.waitForReady(Date.now() + 5000, error => {
    if (error) {
      console.error(
        `❌ TicketService: Failed to connect to User service:`,
        error.message
      )
    } else {
      console.log(
        `✅ TicketService: Successfully connected to User service at ${USER_SERVICE_ADDRESS}`
      )
    }
  })
}
...
setInterval(() => {
  userServiceClient.waitForReady(Date.now() + 3000, error => {
    if (error) {
      console.warn(
        `⚠️ TicketService: User service connection check failed:`,
        error.message
      )
    } else {
      console.log(`🔄 TicketService: User service connection healthy`)
    }
  })
