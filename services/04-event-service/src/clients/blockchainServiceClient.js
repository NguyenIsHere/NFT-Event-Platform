// src/clients/blockchainServiceClient.js (trong 04-event-service)
const grpc = require('@grpc/grpc-js')
const protoLoader = require('@grpc/proto-loader')
const path = require('path')

// Đảm bảo .env được load để lấy BLOCKCHAIN_SERVICE_ADDRESS
// Giả sử server.js của event-service đã load .env rồi, hoặc bạn có thể load ở đây nếu cần:
...
if (!process.env.BLOCKCHAIN_SERVICE_ADDRESS) {
  console.warn(
    "WARNING for EventService's BlockchainClient: BLOCKCHAIN_SERVICE_ADDRESS is not defined in .env. Calls to Blockchain service will likely use default 'localhost' and may fail in Docker."
  )
}
...
console.log(
  `EventService: Blockchain client attempting to connect to ${BLOCKCHAIN_SERVICE_ADDRESS} using proto: ${BLOCKCHAIN_PROTO_PATH}`
)
